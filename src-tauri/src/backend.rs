...
use url::Url;
use uuid::Uuid;
use crate::features::{self, ADVANCED_SYNC, QUOTA_INTELLIGENCE, TOKEN_STATS, USAGE_RECORDS};
...
fn date_range(from: Option<String>, to: Option<String>, timezone: &str) -> Result<Option<(String, String, i64)>, String> {
    match (from, to) {
        (None, None) => Ok(None),
        (Some(from), Some(to)) => {
            let start_date=NaiveDate::parse_from_str(&from,"%Y-%m-%d").map_err(|_|"from must use YYYY-MM-DD")?;
            let end_date=NaiveDate::parse_from_str(&to,"%Y-%m-%d").map_err(|_|"to must use YYYY-MM-DD")?;
            if start_date>end_date { return Err("from must not be after to".into()); }
            let days=(end_date-start_date).num_days()+1;
            if days>3660 { return Err("custom range is too large".into()); }
            let zone=parse_timezone(timezone);
            let start=zone.from_local_datetime(&start_date.and_hms_opt(0,0,0).expect("valid midnight")).earliest().ok_or("invalid from date")?;
            let end=zone.from_local_datetime(&(end_date+Duration::days(1)).and_hms_opt(0,0,0).expect("valid midnight")).earliest().ok_or("invalid to date")?;
            Ok(Some((start.with_timezone(&Utc).to_rfc3339_opts(SecondsFormat::Secs,true),end.with_timezone(&Utc).to_rfc3339_opts(SecondsFormat::Secs,true),days)))
        }
...
fn daily_stats_range(conn:&Connection,from:&str,to:&str,account:Option<&str>,timezone:&str)->Result<Vec<Value>,String>{
    let mut args=vec![json!(timezone),json!(from),json!(to)]; let extra=if let Some(id)=account{args.push(json!(id));" AND account_id=?"}else{""};
    query_json(conn,&format!(r#"SELECT tz_date(created_at,?) date,ROUND(SUM(cost_usd),6) total_cost_usd,ROUND(SUM(cost_usd*COALESCE((SELECT 60.0/NULLIF(mt.monthly_quota_usd,0) FROM model_quota_tiers mt WHERE mt.model_key=normalize_model(model)),1.0)),6) equivalent_cost_usd,COUNT(*) request_count,SUM(input_tokens+cache_read_tokens+cache_write_5m_tokens+cache_write_1h_tokens) total_input_tokens,SUM(input_tokens) uncached_input_tokens,SUM(cache_read_tokens) cache_hit_tokens,SUM(cache_write_5m_tokens+cache_write_1h_tokens) cache_write_tokens,SUM(output_tokens) total_output_tokens,SUM(reasoning_tokens) total_reasoning_tokens FROM usage_records WHERE created_at>=? AND created_at<?{extra} GROUP BY 1 ORDER BY 1 DESC"#),&args)
}
...
fn model_stats_range(conn:&Connection,from:&str,to:&str,account:Option<&str>)->Result<Vec<Value>,String>{
    let mut args=vec![json!(from),json!(to)]; let extra=if let Some(id)=account{args.push(json!(id));" AND account_id=?"}else{""};
    query_json(conn,&format!(r#"SELECT model,SUM(input_tokens+cache_read_tokens+cache_write_5m_tokens+cache_write_1h_tokens) total_input_tokens,SUM(input_tokens) uncached_input_tokens,SUM(cache_read_tokens) cache_hit_tokens,SUM(cache_write_5m_tokens+cache_write_1h_tokens) cache_write_tokens,SUM(output_tokens) total_output_tokens,SUM(reasoning_tokens) total_reasoning_tokens,ROUND(SUM(cost_usd),6) total_cost_usd,COUNT(*) request_count FROM usage_records WHERE created_at>=? AND created_at<?{extra} GROUP BY model ORDER BY total_cost_usd DESC"#),&args)
}
...
async fn route(state:&BackendState, request:&ApiRequest)->Result<Value,String>{
    ...
    match (request.method.as_str(),path) {
      ...
      ("GET","/analytics/opencode/daily")=>{require_feature(state, TOKEN_STATS)?;let days=qp("days").and_then(|v|v.parse().ok()).unwrap_or(30);let range=date_range(qp("from"),qp("to"),&timezone)?;let stats=if let Some((from,to,_))=range.as_ref(){daily_stats_range(&conn,from,to,qp("account_id").as_deref(),&timezone)?}else{daily_stats(&conn,days,qp("account_id").as_deref(),&timezone)?};Ok(json!({"days":days,"from":range.as_ref().map(|v|v.0.clone()),"to":range.as_ref().map(|v|v.1.clone()),"timezone":timezone,"stats":stats}))},
      ("GET","/analytics/opencode/model-tokens")=>{let days=qp("period").map(|p|period_days(&p)).or_else(||qp("days").and_then(|v|v.parse().ok())).unwrap_or(30);let range=date_range(qp("from"),qp("to"),&timezone)?;let stats=if let Some((from,to,_))=range.as_ref(){model_stats_range(&conn,from,to,qp("account_id").as_deref())?}else{model_stats(&conn,days,qp("account_id").as_deref(),&timezone)?};let stats=crate::model_quota::with_equivalent_cost(&conn,&stats)?;let equivalent_cost=crate::model_quota::equivalent_cost_total(&stats);Ok(json!({"days":days,"from":range.as_ref().map(|v|v.0.clone()),"to":range.as_ref().map(|v|v.1.clone()),"timezone":timezone,"stats":stats,"equivalent_cost_usd":equivalent_cost}))},
      ...
      ("GET","/dashboard")=>{let period=qp("period").unwrap_or_else(||"30d".into());ensure_quota_refresh(state,&conn)?;let quota=cached_quota(&conn)?;let days=period_days(&period);let recent=list_usage(&conn,None,0,10)?;let model=crate::model_quota::with_equivalent_cost(&conn,&model_stats(&conn,days,None,&timezone)?)?;let equivalent_cost=crate::model_quota::equivalent_cost_total(&model);let smart=feature_enabled(state,QUOTA_INTELLIGENCE)?;let intel=if smart{quota_intelligence(&conn,None)?}else{Vec::new()};Ok(json!({"overview":{"opencode":aggregate_quota(&quota)},"quota":quota,"recent_usage":{"records":recent["records"],"total":recent["total"]},"model_tokens":model,"equivalent_cost_usd":equivalent_cost,"data_health":health(&conn)?,"quota_intelligence":intel,"period":period,"timezone":timezone}))},
...
mod tests {
  ...
  fn feature_guard_rejects_disabled_groups() {
    ...
    assert!(require_feature(&state,QUOTA_INTELLIGENCE).is_err());
    state.settings.lock().unwrap()["feature_flags"][QUOTA_INTELLIGENCE]=json!(true);
    assert!(require_feature(&state,QUOTA_INTELLIGENCE).is_ok());
  }
  #[test]
  ...
  #[test]
  fn custom_date_range_respects_local_midnight_bounds() {
    let range=date_range(Some("2026-01-01".into()),Some("2026-01-02".into()),"Asia/Shanghai").unwrap().unwrap();
    assert_eq!(range.0,"2025-12-31T16:00:00Z");
    assert_eq!(range.1,"2026-01-02T16:00:00Z");
    assert_eq!(range.2,2);
    assert!(date_range(Some("2026-01-03".into()),Some("2026-01-01".into()),"UTC").is_err());
    assert!(date_range(Some("2026-01-01".into()),None,"UTC").is_err());
  }
  #[test]
