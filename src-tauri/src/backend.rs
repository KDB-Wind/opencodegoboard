...
fn account_view(row: Value) -> Value {
    ...
    let configured = object.get("auth_cookie").and_then(Value::as_str).is_some_and(|v| !v.is_empty());
    let masked = object.get("auth_cookie").and_then(Value::as_str).map(|v| if v.len() > 8 { format!("{}…{}", &v[..4], &v[v.len()-4..]) } else { "***".into() }).unwrap_or_default();
...
fn quota_accounts(conn:&Connection)->Result<Vec<QuotaAccount>,String>{Ok(query_json(conn,"SELECT id,name,workspace_id,auth_cookie,show_rolling,show_weekly,show_monthly FROM opencode_accounts WHERE enabled=1 ORDER BY created_at",&[])?.into_iter().map(|v|QuotaAccount{id:v["id"].as_str().unwrap_or_default().into(),name:v["name"].as_str().unwrap_or_default().into(),workspace_id:v["workspace_id"].as_str().unwrap_or("Default").into(),auth_cookie:v["auth_cookie"].as_str().unwrap_or_default().into(),show_rolling:v["show_rolling"].as_i64().unwrap_or(1)!=0,show_weekly:v["show_weekly"].as_i64().unwrap_or(1)!=0,show_monthly:v["show_monthly"].as_i64().unwrap_or(1)!=0}).collect())}
...
async fn route(state:&BackendState, request:&ApiRequest)->Result<Value,String>{
    ...
    match (request.method.as_str(),path) {
      ...
      ("PUT","/config")=>{let body=request.body.clone().unwrap_or(json!({}));let mut guard=state.settings.lock().map_err(|_|"settings lock poisoned")?;merge_json(&mut guard,&body);drop(guard);config(state,&conn)},
      ("GET","/accounts/opencode")=>Ok(Value::Array(accounts(&conn)?)),
      ("POST","/accounts/opencode")=>{let b=request.body.as_ref().ok_or("request body required")?;let id=Uuid::new_v4().to_string();let timestamp=now();conn.execute("INSERT INTO opencode_accounts(id,name,workspace_id,auth_cookie,show_rolling,show_weekly,show_monthly,enabled,created_at,updated_at) VALUES(?,?,?,?,1,1,1,1,?,?)",params![id,b["name"].as_str().unwrap_or("Account"),b["workspace_id"].as_str().unwrap_or("Default"),b["auth_cookie"].as_str().unwrap_or(""),timestamp,timestamp]).map_err(|e|e.to_string())?;conn.execute("INSERT INTO usage_sync_state(account_id) VALUES(?)",params![id]).map_err(|e|e.to_string())?;Ok(account_view(one(&conn,"SELECT * FROM opencode_accounts WHERE id=?",&[json!(id)])?))},
      ...
      ("GET","/dashboard")=>{let period=qp("period").unwrap_or_else(||"30d".into());drop(conn);let quota=fetch_quotas(state).await?;let conn=Connection::open(&state.db_path).map_err(|e|e.to_string())?;let days=period_days(&period);let recent=list_usage(&conn,None,0,10)?;let model=model_stats(&conn,days,None)?;Ok(json!({"overview":{"opencode":aggregate_quota(&quota)},"quota":quota,"recent_usage":{"records":recent["records"],"total":recent["total"]},"model_tokens":model,"data_health":health(&conn)?,"quota_intelligence":[],"quota_reconciliation":[],"quota_units":{"period":period,"total_quota_units":0,"request_count":0,"models":[]},"recommendations":{"account":Value::Null,"model":Value::Null,"generated_at":now()},"period":period}))},
...
fn overview(conn:&Connection)->Result<Value,String>{let totals=one(conn,"SELECT ROUND(COALESCE(SUM(cost_usd),0),6) total_cost_usd,COUNT(*) total_requests,COALESCE(SUM(input_tokens+cache_read_tokens+cache_write_5m_tokens+cache_write_1h_tokens),0) total_input_tokens,COALESCE(SUM(output_tokens),0) total_output_tokens,COALESCE(SUM(reasoning_tokens),0) total_reasoning_tokens FROM usage_records",&[])?;Ok(json!({"opencode":{"avg_effective_remaining":0,"account_count":accounts(conn)?.len(),"success_count":0,"blocked_count":0,"accounts":[],"bottleneck":Value::Null},"usage":totals}))}
...
fn dynamic_route(conn:&Connection,request:&ApiRequest,path:&str,url:&Url)->Result<Value,String>{
    ...
    if segments.len()>=3&&segments[0]=="accounts"&&segments[1]=="opencode"{let id=segments[2];
      if segments.len()==3&&request.method=="DELETE"{return Ok(json!({"ok":conn.execute("DELETE FROM opencode_accounts WHERE id=?",params![id]).map_err(|e|e.to_string())?>0}));}
      if segments.len()==3&&request.method=="PUT"{let body=request.body.as_ref().ok_or("request body required")?;for key in ["name","workspace_id","auth_cookie"]{if let Some(v)=body.get(key).and_then(Value::as_str){conn.execute(&format!("UPDATE opencode_accounts SET {key}=?,updated_at=? WHERE id=?"),params![v,now(),id]).map_err(|e|e.to_string())?;}}for key in ["show_rolling","show_weekly","show_monthly","enabled"]{if let Some(v)=body.get(key).and_then(Value::as_bool){conn.execute(&format!("UPDATE opencode_accounts SET {key}=?,updated_at=? WHERE id=?"),params![v as i64,now(),id]).map_err(|e|e.to_string())?;}}return Ok(account_view(one(conn,"SELECT * FROM opencode_accounts WHERE id=?",&[json!(id)])?));}
