...
async fn route(state:&BackendState, request:&ApiRequest)->Result<Value,String>{
    ...
    match (request.method.as_str(),path) {
      ...
      ("GET","/analytics/opencode/model-tokens")=>{let days=qp("period").map(|p|period_days(&p)).or_else(||qp("days").and_then(|v|v.parse().ok())).unwrap_or(30);let range=date_range(qp("from"),qp("to"),&timezone)?;let stats=if let Some((from,to,_))=range.as_ref(){model_stats_range(&conn,from,to,qp("account_id").as_deref())?}else{model_stats(&conn,days,qp("account_id").as_deref(),&timezone)?};let equivalent_cost=crate::model_quota::equivalent_cost(&conn,&stats)?;Ok(json!({"days":days,"from":range.as_ref().map(|v|v.0.clone()),"to":range.as_ref().map(|v|v.1.clone()),"timezone":timezone,"stats":stats,"equivalent_cost_usd":equivalent_cost}))},
