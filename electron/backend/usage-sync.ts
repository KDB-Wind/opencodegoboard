...
async function fetchAndInsertBatch(
  ...
  // Never write pages after a failed page. Otherwise deepest_page_fetched can
  // jump across a hole and that missing history may never be retried.
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    const records = result as ParsedUsageRecord[];
    if (!records.length) {
      return {
...
export async function syncUsage(
  ...
  try {
    ...
    while (page - startPage < pagesLimit) {
      ...
      if (batch.lastCompletedPage != null) {
        deepest = Math.max(deepest, batch.lastCompletedPage);
...
export async function backfillUsage(
  ...
  try {
    ...
    while (page - startPage < pagesLimit) {
      ...
      if (batch.lastCompletedPage != null) {
        db.updateUsageSyncState(account.id, {
          deepest_page_fetched: batch.lastCompletedPage,
        });
        page = batch.lastCompletedPage + 1;
      }
      if (batch.exhausted || batch.lastCompletedPage == null) break;
